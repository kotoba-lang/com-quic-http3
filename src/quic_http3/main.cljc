(ns quic-http3.main
  "Kotodama WASM entrypoint for the quic_http3 clean-room actor (L5) — Clojure port.
...
(def entity-specs
  [{:entity "QuicPacket"    :plural "quicpackets"    :id-prefix "quichttp_qui"
    :fields [:packetType :version :sourceConnectionId :destinationConnectionId :payloadLength :packetNumber]
    :required [:packetType :version]
    :coerce {:version :int :payloadLength :int :packetNumber :int}
    :refs {}}
   {:entity "QuicFrame"     :plural "quicframes"     :id-prefix "quichttp_qui"
    :fields [:frameType :payload :streamId :offsetInStream]
    :required [:frameType :payload]
    :coerce {:frameType :int :streamId :int :offsetInStream :int}
    :refs {}}
   {:entity "Http3Frame"    :plural "http3frames"    :id-prefix "quichttp_htt"
    :fields [:frameType :length :payload]
    :required [:frameType :length]
    :coerce {:frameType :int :length :int}
    :refs {}}
   {:entity "Http3Settings" :plural "http3settingses" :id-prefix "quichttp_htt"
    :fields [:identifier :value]
    :required [:identifier :value]
    :coerce {:identifier :int :value :int}
    :refs {}}
   {:entity "QuicConnection" :plural "quicconnections" :id-prefix "quichttp_qui"
    :fields [:connectionId :version :state :lastActivityAt]
...
(defn emit-facts
  "EAVT facts for one record: {\"quic_http3.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "quic_http3-compat" :tier tier :entities entities} 200])
...
(defn start! [] :quic_http3-compat/ready)
